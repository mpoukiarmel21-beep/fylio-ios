## §6 — `FylioCore/FylioSession.swift` — **VERSION CORRIGÉE INTÉGRALE**
...
public actor FylioSession {
    ...
    public func acceptIncoming(on conn: NWConnection,
        ...
        let accepted = onIncoming(announce.sender, announce.manifests, announce.totalBytes)
...
// MARK: - FileHandle util (iOS 13.4+ fournit write(contentsOf:) — aucune extension dupliquée)
```

### Corrections par rapport au brouillon précédent
| Brouillon | Corrigé en |
|---|---|
| `FylioFrameKeyCodec.decodeControl` | `FylioFrameCodec.decodeControl` |
| `renamesBytesByFile` | `receivedBytesByFile` |
| `etry FylioFrameCodec.encodeControl` | `try FylioFrameCodec.encodeControl` |
| `timeInsideIntervalSince1970` | `timeIntervalSince1970` |
| Extension `FileHandle.write(contentsOf:)` dupliquée | Supprimée (API native iOS 13.4+) |
| `FylioStorage` référencé sans paramètre | Injecté explicitement (dépendance propre) |
| Clé de session dérivée d'une empreinte texte | Clé passée par l'appelant (dérivée au bon endroit : handshake QR/mTLS) |
| `computeResumeMap` fantôme | `storage.partialReceipts` injecté |
