...
public final class FylioCrypto: @unchecked Sendable {
    ...
    public static func buildManifest(for file: FylioFileItem,
        ...
        while offset < length {
            ...
            blocks.append(.init(index: blocks.count, offset: offset            let digest = SHA256.hash(data: chunk)
...
## §4 — `FylioCore/FylioDiscovery.swift`
...
public actor FylioDiscovery {
    ...
    public func startAdvertising(port: UInt16) async throws {
        ...
        // TXT : métadonnées publiques uniquement (nom, numéro, plateforme, empreinte).
        let txt: NWTXTRecord = [
            "v": "1",
        ...
        listener.serviceRegistrationUpdateHandler = { change in
            if case .update(let txt) = change {
                // met à jour le TXT si l'identité change (rename)
                _ = txt
            }
        }
    ...
    private func handleResults(_ results: Set<NWBrowser.Result>) {
        for result in results {
            ...
            guard let txt = result.metadata?[(.txt)] as? NWTXTRecord ?? txtFrom(result) else { continue }
            let peer = peerFromTXT(name: name, txt: txt, endpoint: result.endpoint)
            if let peer { onPeerFound?(peer) }
        }
    }

    private func txtFrom(_ result: NWBrowser.Result) -> NWTXTRecord? {
        // Network.framework : le TXT arrive via result.metadata (NWBrowseResult.Metadata)
        if case let .bonjour(txt)? = result.metadata { return txt }
        return nil
    }

    private func peerFromTXT(name: String, txt: NWTXTRecord, endpoint: NWEndpoint) -> FylioPeer? {
        guard let num = txt["num"] as? String, !num.isEmpty,
              let fp = txt["fp"] as? String, !fp.isEmpty,
              let devID = UUID(uuidString: String(name.split(separator: ".").first ?? "")) else { return nil }
        let pf = FylioPlatform(rawValue: (txt["pf"] as? String) ?? "ios") ?? .ios
        return FylioPeer(id: devID,
                         displayName: (txt["name"] as? String) ?? num,
                         fylioNumber: num,
                         avatarID: Int(txt["av"] as? String ?? "1") ?? 1,
                         platform: pf,
                         host: "bonjour",
                         port: 0,
                         fingerprint: fp,
