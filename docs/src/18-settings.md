# FYLIO — `FylioUI/Screens/SettingsView.swift` (page Paramètres complète)
...
> Note : `app.clearCache()` / `app.eraseAllData()` sont ajoutés dans le bloc AppViewModel (petites méthodes dédiées, fichier séparé).
